...
Code baseline: `main` at `e50b99837030618913b040baca76bac023bbaf9b`.
Implementation branch: `codex/record-telegram-outbox-cloud`.
Scope: record the applied and verified durable Telegram reply outbox after completing Cloud privilege hardening. No secret value is included.
...
| Database | Eleven migrations under `supabase/migrations/`; generated types; RLS statements present | The managed Drizzle journal records nine applied entries through `0008`; `0008` is Lovable's semantically identical managed duplicate of the outbox migration |
...
3. **Webhook durability:** merged `main` stores the authenticated raw update before returning success and queues outbound replies durably by update/action key. Replies become deliverable only after command processing succeeds and retry independently with leases, bounded backoff and dead-letter handling. The Cloud migration, grants, functions, transactional behavior and expanded same-job scheduler predicate are verified; see the [outbox rollout record](operations/2026-09-12-telegram-outbox.md). Telegram has no general idempotency key, so a crash after Telegram accepts an action but before `sent_at` is stored remains an explicit duplicate-delivery window. A real Telegram update/reply journey is still pending.
