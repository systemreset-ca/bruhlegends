...
3. **Webhook durability:** merged `main` stores the authenticated raw update before returning success. The current branch adds a durable outbound outbox: webhook replies are keyed by update/action, become deliverable only after command processing succeeds, and retry independently with leases, bounded backoff and dead-letter handling. This prevents Telegram API failures from rerunning command database effects. Telegram has no general idempotency key, so a crash after Telegram accepts an action but before `sent_at` is stored remains an explicit duplicate-delivery window. The migration and live end-to-end preview test are still pending.
