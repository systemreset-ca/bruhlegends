...
[Live public results](twelve-account-devnet-results.json) are separate from the synthetic-proof fixture. Initial public faucet attempt returned RPC error -32603; zero live transfers had occurred at that point. The in-memory root address is `63fHxpd6cAoHGDqdCK3EZLfjv9Lgut9NHyuqv3CeSxa3`. A single bounded Helius funding request in the original Lovable project verified devnet genesis, then was refused as rate_limited, without a signature or application/config changes. QuickNode's independent web form rejected the unfunded address for lack of mainnet balance despite conflicting FAQ wording; no mainnet funds were sent. The official Solana faucet was opened and prefilled for an owner-operated request/verification. Final funding outcome must be recorded before labeling the live chain portion successful.
