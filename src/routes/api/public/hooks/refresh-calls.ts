      ...
      POST: async ({ request }) => {
        ...
        for (const hit of result.milestones) {
          const { data: call } = await db
            .from("calls")
            .select("symbol, mint, group_id, groups(telegram_chat_id, announce_tips, is_paused), group_members(display_name)")
            .eq("id", hit.callId)
            .maybeSingle();
          if (!call?.groups || call.groups.is_paused) continue;

          await sendMessage(
            call.groups.telegram_chat_id,
            [
